...
def downloadimage(recipe_name):
    ...
    #"output_directory": "downloaded_images": This specifies the directory where you want to save the downloaded images
    ...
    for i, filename in enumerate(os.listdir(parent_dir)): #parentg_dir contains multiple files with various receipes names iterating thorough them
        #this loop renames each file in parent_dir to include the recipe_name followed by a unique index and ".jpg" extension.
        dst = recipe_name + str(i) + ".jpg"
        src = parent_dir + filename
        dst1 = parent_dir + dst
        os.rename(src, dst1)
        for j in range(12): #For each recipe 10 images are downloaded for training and 2 for testing. hence 12
            shutil.copy(dst1, images) #from dst1 all images are being copied to IMAGES FOLDER
            if i <= 9:
                shutil.copy(dst1, train_dir)  #from dst1 all the tarining images(10) are being copied to training folder
            else:
                shutil.copy(dst1, test_dir)   #from dst1 all the testing images(2) are being copied to testing folder
...
def getname(raw_text):
    ...
    return raw_text.strip() #removes leading and trailing whitespaces in the raw_text
...
if __name__ == "__main__":
    links = pd.read_csv("links.csv") # list of links stored in a CSV file
